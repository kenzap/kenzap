...
export function devApp(id, cmd) {
    ...
    if (cmd == 'start') {
        ...
        let cb = () => {
            ...
            console.log('cd ' + cache.path + ' && devspace sync --config=devspace.yaml --kubeconfig=' + kubeconfig)
        }

        global.state.dev[id].status = "sync";
        global.state.dev[id].proc = run_script('cd ' + cache.path + ' && rm -Rf .devspace && devspace sync --config=devspace.yaml --kubeconfig=' + kubeconfig, [], cb);
    }
    ...
    if (cmd == 'stop') {
        ...
        terminate(global.state.dev[id].proc.pid, err => console.log(err))
...
export function checkConfigs(id) {
    ...
    if (fs.existsSync(path.join(cache.path, 'app.yaml'))) {
        ...
        console.log('app.yaml does not exist');
    }

    // kubeconfig-sg.yaml
    if (fs.existsSync(path.join(cache.path, 'kubeconfig-sg.yaml'))) {

        console.log('kubeconfig-sg.yaml exists');
    } else {

        console.log('kubeconfig-sg.yaml does not exist');
    ...
    if (fs.existsSync(path.join(cache.path, 'devspace.yaml'))) {
        ...
        console.log('devspace.yaml exists');
    } else {

        console.log('devspace.yaml does not exist');
...
export function deploy(id) {
    ...
    if (cache.path) if (fs.existsSync(path.join(cache.path, 'devspace.yaml'))) {
        ...
        try {
            ...
            console.log('deploy');
            console.log('devspace deploy -n ' + id + ' --config=devspace.yaml --kubeconfig=' + kubeconfig + ' -b');
    ...
    let step5 = () => {
        ...
        if (fs.existsSync(kenzapFilePath)) {
            fs.unlink(kenzapFilePath, (err) => {
                if (err) {
                    console.log('Error removing .kenzap file:', err);
                } else {
                    console.log('.kenzap file removed successfully');
                }
            });
        } else {
            console.log('.kenzap file does not exist');
        }
...
export function checkEnvironment() {
    ...
    let status = { c: 0, allow: true, svc: [] }
    let cb = () => { };
    let callback_response = (data) => {

        data = data.toString();

        status.c++;

        if (data.indexOf('can not create home directory') >= 0) { status.allow = false; status.svc.push({ name: "Can not create home directory", link: "https://kenzap.com" }) }

        if (data.indexOf('docker: command not found') >= 0 || data.indexOf('docker: not found') >= 0 || data.indexOf('not found: docker') >= 0 || data.indexOf('Cannot connect to the Docker') >= 0) { status.allow = false; status.svc.push({ name: "Docker Desktop", link: "https://docs.docker.com/desktop/install/mac-install/" }) }

        if (data.indexOf('kubectl: command not found') >= 0 || data.indexOf('kubectl: not found') >= 0 || data.indexOf('not found: kubectl') >= 0) { status.allow = false; status.svc.push({ name: "kubectl", link: "https://kubernetes.io/docs/tasks/tools/install-kubectl-macos/" }) }

        if (data.indexOf('devspace: command not found') >= 0 || data.indexOf('devspace: not found') >= 0 || data.indexOf('not found: devspace') >= 0) { status.allow = false; status.svc.push({ name: "DevSpace", link: "https://www.devspace.sh/docs/getting-started/installation?x0=1" }) }

        // show warning message, block UI
        if (status.c >= 4 && !status.allow) {
                ...
                <div class="container">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="ms-sm-2 ms-0 dropdown d-none">
                            ...
                            <ul class="dropdown-menu spaceSelectList" aria-labelledby="spaceSelect"><li><a data-id="1000000" class="spw dropdown-item" href="#">K-Space</a></li><li><a data-id="1000409" class="spw dropdown-item" href="#">Pasha</a></li><li><a data-id="1000452" class="spw dropdown-item" href="#">Daniel Demo</a></li><li><a data-id="1001819" class="spw dropdown-item" href="#">World Athletics Relays 2023</a></li><li><a data-id="1001964" class="spw dropdown-item" href="#">SICC </a></li><li><a data-id="1002078" class="spw dropdown-item" href="#">Mookata</a></li><li><a data-id="1002170" class="spw dropdown-item" href="#">Skarda Design</a></li><li><a data-id="1002968" class="spw dropdown-item" href="#">ALLEAZE Tech</a></li><li><a data-id="1002991" class="spw dropdown-item" href="#">lakshan ks</a></li><li><a data-id="1003376" class="spw dropdown-item" href="#">Concert Hall</a></li><li><a data-id="1003603" class="spw dropdown-item" href="#">MyTicket</a></li><li><hr class="dropdown-divider"></li><li><a data-id="manage" class="spw dropdown-item" href="#">Manage</a></li></ul>
                        </div>
                    </div>
            ...
            <div class="container mt-3" >
                <h2>${__html('Welcome!')}</h2>
                <p>${__html('Please make sure the following dependencies are running:')}</p>
                <ul>
                    ${status.svc.map(s => {

                return `<li class="open-dep-link po" data-link="${attr(s.link)}">${s.name}</li>`

            }).join('')
                }
                </ul>
                <div class="row" style="height:50%; display:block; margin:0px auto 0 auto;">
                    <button type="button" class="btn btn-primary btn-lg btn-refresh position-absolute" style="width:90%;bottom:32px;margin-left: auto;margin-right: auto;left: 0;right: 0;text-align: center;">${__html("Refresh")}</button>
                </div>
            </div>
            `;
        }
    ...
    if (!fs.existsSync(kenzapdir)) { try { fs.mkdirSync(kenzapdir); } catch (e) { parseError(e) } }
    if (!fs.existsSync(kenzapdir)) { callback_response("can not create home directory"); }
    run_script('docker stats --no-stream', [], cb, 0, callback_response);
    run_script('type kubectl', [], cb, 0, callback_response);
    run_script('type devspace', [], cb, 0, callback_response);
}
...
export function syncDeployments(data) {
    ...
    let cb = () => {
        ...
        console.log('syncing data centers');
...
export function run_script(command, args, callback, verbose = 1, callback_error, callback_output) {
    ...
    child.stdout.on('data', (data) => {
        ...
        console.log("script child stdout data");
        console.log(data);
    ...
    child.stderr.on('data', (data) => {
        ...
        console.log("script child stderr data");
        console.log(data);
