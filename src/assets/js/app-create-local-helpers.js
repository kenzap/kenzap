 ...
 */
export function provisionClusterAppLocal(app, cb) {

    log('Provisioning cluster local app: ', app.id);
    ...
    let step1 = () => {
        ...
        // clear namespace
        log(`kubectl config use-context minikube && cd ${app.path}; kubectl delete all --all -n ${app.slug}`);
        run_script(`kubectl config use-context minikube && cd ${app.path}; kubectl delete all --all -n ${app.slug}`, [], () => { consoleUI(`Removing previous resources in ${app.slug} namespace`); step2(); }, 0, (error) => { log('Cluster 1 E: ', error.toString()); });
    }

    let step2 = () => {

        // clear namespace
        run_script(`kubectl config use-context minikube && cd ${app.path}; kubectl delete namespace ${app.slug}`, [], () => { consoleUI(`Removing ${app.slug} namespace`); step3(); }, 0, (error) => { log('Cluster 2 E: ', error.toString()); step3(); });
    }

    let step3 = () => {

        // force delete namespace and all resources
        log(`kubectl config use-context minikube && kubectl get namespace "${app.slug}" -o json | tr -d "\n" | sed "s/\"finalizers\": \[[^]]\+\]/\"finalizers\": []/" | kubectl replace --raw /api/v1/namespaces/${app.slug}/finalize -f -`);
        run_script(`kubectl config use-context minikube && cd ${app.path}; kubectl get namespace "${app.slug}" -o json | tr -d "\n" | sed "s/\"finalizers\": \[[^]]\+\]/\"finalizers\": []/" | kubectl replace --raw /api/v1/namespaces/${app.slug}/finalize -f -`, [], () => { step4(); consoleUI('Namespace force removed'); }, 0, (error) => { step4(); log('Cluster 3 E: ', error.toString()); });
    }
    ...
    let step4 = () => {
        ...
        log(`kubectl config use-context minikube && kubectl delete CertificateSigningRequest ${app.slug}`);
        run_script(`kubectl config use-context minikube && kubectl delete CertificateSigningRequest ${app.slug}`, [], () => { consoleUI('Clearing previous certificate signing requests'); step5(); }, 1, (error) => { step5(); });
    }
 ...
 */
export function provisionClusterUserLocal(app, cb) {
 ...
 * createLocalApp(app);
 */
export function createLocalAppLocal(app, cb) {
    ...
    let template_endpoint = app.slug + ".endpoint-" + settings.id.toLowerCase() + ".local";
    ...
    setTimeout(() => {
        ...
        run_script('kubectl config use-context minikube && cd ' + app.path + ' && kubectl apply -f endpoints.yaml', [], () => { });
    }, 2000);
...
export function applyActions(app, targetPath) {
    ...
    // app minikube setings
    fileContent = fileContent.replace(/.*cert-manager\.io\/cluster-issuer:.*\n?/g, '');
    fileContent = fileContent.replace(/.*ingress.kubernetes.io\/force-ssl-redirect:.*\n?/g, '');
    fileContent = fileContent.replace(/.*tls:\n\s*-\s*hosts:\n\s*- template_endpoint\n\s*secretName: letsencrypt-prod.*\n?/g, '');

    // Replace imagePullPolicy: Always with imagePullPolicy: Never
    fileContent = fileContent.replace(/imagePullPolicy:\s*Always/g, 'imagePullPolicy: Never');

    // Replace localRegistry: enabled: false with localRegistry: enabled: true
    fileContent = fileContent.replace(/localRegistry:\s*enabled:\s*false/g, 'localRegistry:\n  enabled: true');
